    ...
    // Esta interfaz define EXACTAMENTE lo que devuelve tu Laravel (id, text)
    interface EstudianteOpcion {
    id: number;
    text: string;
    }

    // Esta interfaz define la estructura de una fila en tu formulario
    ...
    const listaEstudiantes = ref<EstudianteOpcion[]>([]);
    ...
    onMounted(async () => {
        ...
        try {
            ...
            notificacion('Error al cargar los datos iniciales.', { type: 'danger', title: 'Error' });
        }
    });

    async function handleSubmit() {
        if (!form.id_tipo_incidencia) {
            notificacion('El tipo de incidencia es obligatorio.', { type: 'danger', title: 'Validación' });
            return;
        }

        if (!form.id_lugar_incidencia) {
            notificacion('El lugar de la incidencia es obligatorio.', { type: 'danger', title: 'Validación' });
            return;
        }

        if (!form.id_area) {
            notificacion('El área a reportar es obligatoria.', { type: 'danger', title: 'Validación' });
            return;
        }

        if (!form.descripcion.trim()) {
            notificacion('La descripción de la incidencia es obligatoria.', { type: 'danger', title: 'Validación' });
            return;
        }
    ...
    /**
     * Función disparada por @update:search
     */
    const buscarEstudiantesServidor = (textoBusqueda: string) => {
        // Validación de tipo y longitud
        if (!textoBusqueda || typeof textoBusqueda !== 'string' || textoBusqueda.length < 3) {
        ...
        debounceTimer = setTimeout(async () => {
            try {
                // Usamos genérico <EstudianteOpcion[]> para que axios sepa qué devuelve
                const response = await apiClient.get<EstudianteOpcion[]>('/configuracion/estudiante/select', {
        ...
        <AuthenticatedLayout>
            <v-container fluid class="pa-4">
                <v-card rounded="xl" elevation="2" class="pa-6">
                                ...
                                <v-card variant="outlined" class="pa-3 rounded-lg">
                                    ...
                                    <v-col cols="12" md="10">
                                        <v-autocomplete
                                            ...
                                            :loading="cargando"
                                            @update:search="buscarEstudiantesServidor"
                    ...
                    <div class="d-flex flex-column gap-2"> <v-btn
                            block
                            type="submit"
                            color="primary"
                            variant="flat"
